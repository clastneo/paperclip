...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? "전체 기간 예산" : "월간 UTC 예산";
}
...
export function BudgetPolicyCard({
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>남은 예산</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : "제한 없음"}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "예산을 올리거나 이슈를 해제할 때까지 이 프로젝트의 실행이 일시 중지됩니다."
          : "예산을 올리거나 이슈를 해제할 때까지 이 범위의 하트비트가 일시 중지됩니다."}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? "저장 중..." : summary.amount > 0 ? "예산 업데이트" : "예산 설정"}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? "일시 중지" : summary.status === "warning" ? "경고" : summary.status === "hard_stop" ? "강제 중지" : "정상"}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">0 이상인 올바른 금액을 입력해 주세요.</p>
        ) : null}
      </div>
    );
  }
