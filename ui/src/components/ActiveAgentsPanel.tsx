    ...
    <div>
      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">최근 실행한 에이전트가 없습니다.</p>
        </div>
      ) : (
    ...
    <div className={cn(
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>
                {isActive
                  ? "지금 실행 중"
                  : run.finishedAt
                    ? `${relativeTime(run.finishedAt)}에 종료`
                    : `${relativeTime(run.createdAt)}에 시작`}
              </span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? "트랜스크립트를 정리하는 중입니다..." : isActive ? "출력을 기다리는 중입니다..." : "기록된 트랜스크립트가 없습니다."}
        />
      </div>
    </div>
  );
