...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed" ? "종료 다시 시도" : "워크스페이스 종료";
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? "워크스페이스 종료를 다시 시도했습니다." : "워크스페이스를 종료했습니다.",
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            <span className="font-medium text-foreground">{workspaceName}</span> 워크스페이스를 보관 처리하고, 이 워크스페이스가 소유한 아티팩트를 정리합니다. Paperclip은 워크스페이스 기록과 이슈 이력은 유지하지만, 활성 워크스페이스 목록에서는 제거합니다.
          </DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
            ...
            이 워크스페이스를 안전하게 종료할 수 있는지 확인하는 중입니다...
          </div>
        ) : readinessQuery.error ? (
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : "워크스페이스 종료 가능 상태를 확인하지 못했습니다."}
          </div>
        ) : readiness ? (
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              ...
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? "공유 워크스페이스 세션입니다. 보관 처리하면 이 세션 기록은 제거되지만, 실제 프로젝트 워크스페이스는 유지됩니다."
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ? "이 실행 워크스페이스는 별도 체크아웃 경로를 가지므로 독립적으로 보관 처리할 수 있습니다."
                    : readiness.isProjectPrimaryWorkspace
                      ? "이 실행 워크스페이스는 현재 프로젝트의 기본 워크스페이스 경로를 가리키고 있습니다."
                      : "이 워크스페이스는 임시 성격이므로 보관 처리할 수 있습니다."}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">차단 중인 이슈</h3>
              ...
              <section className="space-y-2">
                ...
                <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">기준 ref</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? "설정되지 않음"}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">기준 브랜치 병합 여부</div>
                      <div>{readiness.git.isMergedIntoBase == null ? "알 수 없음" : readiness.git.isMergedIntoBase ? "예" : "아니요"}</div>
                    </div>
                    <div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">그 밖의 연결된 이슈</h3>
                <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                이 워크스페이스에서는 이전 정리 작업이 실패했습니다. 종료를 다시 시도하면 정리 절차를 다시 실행하고, 성공 시 워크스페이스 상태를 업데이트합니다.
              </div>
            ) : null}

            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
                이 워크스페이스는 이미 보관 처리되었습니다.
              </div>
            ) : null}
