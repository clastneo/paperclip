...
const quickFilterPresets = [
  ...
  { label: "진행 중", statuses: ["todo", "in_progress", "in_review", "blocked"] },
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? "사용자")
  ...
  return (
    <div className="space-y-4">
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="현재 필터 또는 검색 조건과 일치하는 이슈가 없습니다."
          action="이슈 만들기"
                          ...
                          >
                            {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 text-xs">
                                ...
                                {formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? "사용자"}
                              </span>
                            ) : (
